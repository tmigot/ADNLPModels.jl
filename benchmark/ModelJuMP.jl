#=
The issue here is that we cannot access the hessian of the matrix.
By the way, we cannot create the MathOptNLPModel (check issue online).
=#
...
function direct_model(cam_indices::Vector{Int}, pnt_indices::Vector{Int}, pt2d::Matrix, cam_params_init::Matrix, pt3d_init::Matrix)
    ...
    return model #value.(cameras), value.(points)
end
